...
/**
 * Reverse of normalizeHostPath: convert a container-normalized path back to
 * the original host path. When HOST_HOME is set (Docker), paths like
 * /root/Repos/... are rewritten to /Users/<name>/Repos/... so the macOS
 * bridge can open them.
 *
 * If HOST_HOME is not set (native macOS run), the path is returned unchanged.
 */
export function toHostPath(p: string): string {
  const hostHome = process.env.HOST_HOME;
  if (!hostHome) return p;
  const containerHome = homedir();
  if (p === containerHome) return hostHome;
  if (p.startsWith(containerHome + "/")) return hostHome + p.slice(containerHome.length);
  return p;
}
