...
/**
 * Check whether CLI commands and macOS app bundles are installed on the host.
 * Called by the containerized settings API so it can display accurate status.
 *
 * Request body: { commands?: string[], apps?: string[] }
 * Response:     { commands: Record<string, boolean>, apps: Record<string, boolean> }
 */
async function handleCheck(body) {
  const { commands = [], apps = [] } = body;

  const [commandResults, appResults] = await Promise.all([
    Promise.all(
    ...
    Promise.all(
      apps.map(async (appName) => {
        if (!appName) return [appName, false];
        try {
          await execFileAsync("open", ["-Ra", appName], { timeout: 3000 });
          return [appName, true];
        } catch {
  ...
  return {
    ...
    apps: Object.fromEntries(appResults),
  };
}
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/check") {
      try {
        ...
        console.error(`[action-bridge] check error: ${err.message}`);
